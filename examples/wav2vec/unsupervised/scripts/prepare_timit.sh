...
KALDI_ROOT=/lustre/s2196654/pykaldi/tools/kaldi
...
cat $tgt_dir/all_sph.flist | sed -e 's#//*#/#g' -e 's#.*/\([^/]*\)/\([^/]*\).wav#\1_\2#g' | awk '{ print toupper($0) }' > $tgt_dir/all.uid
paste -d' ' $tgt_dir/{all_sph.flist,all.uid} | \
    awk -v sph2wav=$sph2wav -v wav_dir=$wav_dir '{print sph2wav " -f wav " $1 " > " wav_dir "/" toupper($2) ".wav"}' \
  > $tgt_dir/sph2wav.sh
...
cat $tgt_dir/all_phn60.flist | sed -e 's#//*#/#g' -e 's#.*/\([^/]*\)/\([^/]*\).phn#\1_\2#g' | awk '{ print toupper($0) }' | \
